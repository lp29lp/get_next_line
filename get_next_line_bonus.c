...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/06/17 11:45:37 by lpaulo-d          #+#    #+#             */
/*   Updated: 2021/06/17 11:45:37 by lpaulo-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_next_line(int fd, char **line)
{
	...
	while (vr.index > 0)
	{
		vr.buf[vr.index] = '\0';
		if (plus[fd] == NULL)
			plus[fd] = ft_strdup(vr.buf);
		else
		{
			vr.tmp = ft_strjoin(plus[fd], vr.buf);
			free(plus[fd]);
			plus[fd] = vr.tmp;
		}
		if (ft_strchr(plus[fd], '\n'))
			break ;
		vr.index = read(fd, vr.buf, BUFFER_SIZE);
	}
	free(vr.buf);
